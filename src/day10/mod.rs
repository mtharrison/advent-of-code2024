...
pub fn find_trails(
    ...
    for neighbour in grid.neighbours(pos) {
        ...
        let expected = next_height.to_string().chars().nth(0).unwrap();
        if next == &expected {
            if next_height == 9 {
...
pub fn trailhead_rating(grid: &Grid<char>, pos: (usize, usize)) -> usize {
    ...
    if value != &'0' {
        ...
        return distinct_trails(grid, pos, 1);
    }
}
...
mod tests {
    ...
    fn test_example_part1() {
        let grid = util_parse::<Grid<char>>("day10", "example.txt", |s| Grid::from(s));
    ...
    #[test]
    fn test_part_1() {
        let grid = util_parse::<Grid<char>>("day10", "puzzle.txt", |s| Grid::from(s));
        let score = map_score(&grid, trailhead_score);
        assert_eq!(score, 430);
    }
